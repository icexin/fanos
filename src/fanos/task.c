...
static void create_task_tss(struct task_t *t, int (*task_fn)())
{
	...
	tss->fs = 0x17;
	tss->gs = 0x10;
	tss->eip = (uint32)task_fn;
	tss->ldt = _LDT(t->pid);

	/*加入gdt*/
	create_gdt_desc(GDT_TSS(t->pid), (uint32)(tss),
...
static void create_task_ldt(struct task_t *t)
{
	...
	create_ldt_desc(d+1, (uint32)t->start_addr,
		TASK_MEM_LEN, DA_DPL3|DA_32|DA_CR|DA_LIMIT_4K);
	create_ldt_desc(d+2, (uint32)t->start_addr,
		TASK_MEM_LEN, DA_DPL3|DA_32|DA_DRW|DA_LIMIT_4K );

	/*加入gdt*/
	create_gdt_desc(GDT_LDT(t->pid), (uint32)d,
...
struct task_t* create_task0()
{
	...
	create_gdt_desc(GDT_TSS(0), (uint32)tss,
		sizeof(struct task_t) - 1, DA_386TSS);
	return task0;
}
...
{
	return (void*)((uint32)t->start_addr + (uint32)addr);
}
