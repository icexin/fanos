...
static uint32 copy_process_args(struct task_t* t, char* args[]);
	     ...
	     int gs, int fs, int es, int ds, int eip, int cs, int eflags, int esp, int ss)
{
	...
	*t = *current_task;
	...
	create_ldt_desc(t->ldt+1, (uint32)t->start_addr, PROCESS_MEM_LEN, DA_DPL3|DA_32|DA_CR|DA_LIMIT_4K);
	create_ldt_desc(t->ldt+2, (uint32)t->start_addr, PROCESS_MEM_LEN, DA_DPL3|DA_32|DA_DRW|DA_LIMIT_4K);
	create_gdt_desc(GDT_LDT(pid), (uint32)t->ldt, sizeof(struct desc)*3-1, DA_LDT);
...
int sys_exec(char *name, char* args[])
{
	...
	uint32 new_esp = copy_process_args(current_task, args);

	uint32* call_stack = (unsigned int*)&name;
...
uint32 copy_process_args(struct task_t* t, char* args[])
{
	uint32 argc = 0;
	char** user_argv = NULL;
	char* user_arg = NULL;
	...
	uint32 new_argv_user = TASK_SIZE - (argc + 1) * sizeof(uint32*);
	uint32 new_arg_user = new_argv_user;

	uint32* new_argv_kern = get_user_addr(t, (void*)new_argv_user);
	uint32* new_arg_kern = NULL;
	...
	uint32 new_esp_user = new_arg_user - 2 * sizeof(uint32*) - 128;
	uint32* new_esp_kern = get_user_addr(t, (void*)new_esp_user);
	new_esp_kern[0] = argc;
